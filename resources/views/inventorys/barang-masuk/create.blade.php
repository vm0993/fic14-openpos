...
<div class="grid grid-cols-12 gap-10 mt-5">
    <div class="intro-y col-span-12 lg:col-span-12 md:col-span-12 sm:col-span-12">
        @if(!empty($result))
        <form action="{{ route('inventorys.item-incoming.update',['item_incoming'=> $result['id']]) }}"
            method="post" onkeydown="return event.key != 'Enter';">
        @else
        <form action="{{ route('inventorys.item-incoming.store') }}"
            method="post" onkeydown="return event.key != 'Enter';">
        @endif
            @csrf
            @if(!empty($result))
                        ...
                        <div class="intro-y">
                            ...
                            <div class="grid grid-cols-12 gap-x-5 mt-1">
                                ...
                                <div class="col-span-12 lg:col-span-3 sm:col-span-3 mt-1">
                                    <div class="input-group">
                                        <div class="input-group-text w-full text-center">
                                            <span class="text-theme-20">Receive.Qty</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-span-12 lg:col-span-3 sm:col-span-3 mt-1">
                                    <div class="input-group">
                                        <div class="input-group-text w-full text-center">
                                            <span class="text-theme-20">Receive.Price</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
@section('footer')
    ...
    <script type="text/javascript">
        ...
        if(addState=='create'){
            ...
            $.get(url + '/'+ id +'/get-detail',  function(data, status){
                for(var a=0;a<data.length;a++){
                    ...
                    html += '               <option value="0">Select Item Receive</option>';
        ...
        function getReceiveItem(){
            transDate        = $('#transaction_date').val();
            tanggalTransaksi = transDate.split("-").reverse().join("-");
            $.ajax({
                type: 'POST',
                url : "{{ route('inventorys.item-incoming.get-incoming-number') }}",
                data: {
                    _token     : '{{ csrf_token() }}',
