...
<div class="grid grid-cols-12 gap-10 mt-5">
    <div class="intro-y col-span-12 lg:col-span-12 md:col-span-12 sm:col-span-12">
        @if(!empty($result))
        <form action="{{ route('masters.ingradiant.update',['ingradiant'=> $result['id']]) }}"
            method="post" onkeydown="return event.key != 'Enter';">
        @else
        <form action="{{ route('masters.ingradiant.store') }}"
            method="post" onkeydown="return event.key != 'Enter';">
        @endif
            @csrf
            @if(!empty($result))
                        ...
                        <div class="intro-y">
                            ...
                            <div class="grid grid-cols-12 gap-x-5 mt-1">
                                ...
                                <div class="col-span-12 lg:col-span-3 sm:col-span-3 mt-1">
                                    <div class="input-group">
                                        <div class="input-group-text w-full text-center">
                                            <span class="text-theme-20">Qty.Usage</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-span-12 lg:col-span-3 sm:col-span-3 mt-1">
                                    <div class="input-group">
                                        <div class="input-group-text w-full text-center">
                                            <span class="text-theme-20">Cost.Estimate</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-right mt-5">
                                <a href="{{ route('masters.ingradiant.index') }}" class="btn closeModal btn-outline-secondary w-20 mr-1">Cancel</a>
...
@section('footer')
    ...
    <script type="text/javascript">
        ...
        if(addState=='create'){
            ...
            id = $('#id').val();
            $.get(url + '/'+ id +'/detail-ingradiant',  function(data, status){
                for(var a=0;a<data.length;a++){
                    ...
                    html += '               <option value="0">Select Raw Material</option>';
                    ...
                    html += '        <div class="input-group">';
                    html += '            <input type="text" name="qty_usage_'+ a +'" id="qty_usage_'+ a +'" value="'+accounting.toFixed(data[a].qty_usage,2)+'" onkeypress="return isNumberKey(event)" class="form-control number-separator text-right"/>';
                    html += '        </div>';
                    html += '    </div>';
                    html += '    <div class="col-span-12 lg:col-span-3 sm:col-span-3 mt-2">';
                    html += '        <div class="input-group">';
                    html += '            <input type="text" name="cost_usage_'+ a +'" id="cost_usage_'+ a +'" value="'+accounting.toFixed(data[a].cost_usage)+'" onkeypress="return isNumberKey(event)" class="form-control number-separator amount text-right"/>';
                    html += '        </div>';
                    html += '    </div>';
                    html += '    <div class="col-span-12 lg:col-span-12 sm:col-span-12 mt-2">';
                    html += '        <div class="input-group">';
                    html += '            <input type="text" name="note_'+ a +'" id="note_'+ a +'" value="'+data[a].note+'" class="form-control form-control-sm" placeholder="Note Bahan Baku" />';
        ...
        function dynamicField(number){
            ...
            html += '    <div class="col-span-12 lg:col-span-3 sm:col-span-3 mt-2">';
            html += '        <div class="input-group">';
            html += '            <input type="text" name="qty_usage_'+ number +'" id="qty_usage_'+ number +'" onkeypress="return isNumberKey(event)" class="form-control number-separator text-right"/>';
            html += '        </div>';
            html += '    </div>';
            html += '    <div class="col-span-12 lg:col-span-3 sm:col-span-3 mt-2">';
            html += '        <div class="input-group">';
            html += '            <input type="text" name="cost_usage_'+ number +'" id="cost_usage_'+ number +'" onkeypress="return isNumberKey(event)" class="form-control number-separator amount text-right"/>';
            html += '        </div>';
            html += '    </div>';
            html += '    <div class="col-span-12 lg:col-span-12 sm:col-span-12 mt-2">';
            html += '        <div class="input-group">';
            html += '            <input type="text" name="note_'+ number +'" id="note_'+ number +'" class="form-control form-control-sm" placeholder="Note Bahan Baku" />';
            html += '            <button type="button" class="input-group-text remove">';
            html += '                <img src="{{ asset('vendor/blade-lucide-icons/trash-2.svg') }}" class="w-4 h-4"/>';
            html += '            </button>';
            html += '        </div>';
